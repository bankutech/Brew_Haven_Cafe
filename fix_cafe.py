...
with open('cafe.html', 'r', encoding='utf-8') as f:
    html = f.read()
...
# 3. Local assets 404
html = html.replace('assets/about-cafe.png', 'https://images.unsplash.com/photo-1554118811-1e0d58224f24?auto=format&fit=crop&w=600&q=80')
html = html.replace('assets/menu-espresso.png', 'https://images.unsplash.com/photo-1510061164039-bfa22421a221?auto=format&fit=crop&w=600&q=80')
html = html.replace('assets/barista-1.png', 'https://images.unsplash.com/photo-1511920170033-f8396924c348?auto=format&fit=crop&w=400&q=80')
...
if 'window.closeCheckout = ' not in html:
...
def replace_menu_price(match):
    name = match.group(1)
    usd = match.group(2)
    # find the price in ₹ for this item
    # Example snippet: <h3 class="menu-card-title">South Indian Filter Kaapi</h3>\n                <span class="menu-card-price">₹290</span>
    # and: data-item-name="South Indian Filter Kaapi" data-item-price="3.50"
    return f'data-item-name="{name}" data-item-price="{usd}"'
...
for name, price in price_map.items():
    html = re.sub(rf'data-item-name="{name}"\s+data-item-price="[0-9.]+"', f'data-item-name="{name}" data-item-price="{price}"', html)
...
html = html.replace('href="restaurant.html#reservations"', 'href="#contact"')

# 11. Success modal close reset state
# Wait, let me add a generic reset script
script_addition = """
<script>
...
# 17. Team aria labels
html = html.replace('Liam\'s', 'Rohan\'s')
html = html.replace('Sophia\'s', 'Ananya\'s')
html = html.replace('Daniel\'s', 'Vikram\'s')
...
# Write back to file
with open('cafe.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Done fixing cafe.html")
